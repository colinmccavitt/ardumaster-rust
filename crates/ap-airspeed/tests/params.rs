...
fn airspeed_params_defaults_match_upstream_ratio() {
    ...
    assert_eq!(params.airspeed1.tube_order, 2);
    ...
    assert!((params.wind_warn - ARSPD_WIND_WARN_DEFAULT).abs() < 1e-6);
    assert_eq!(params.wind_warn(), 0.0);
    assert!((params.fbw_min - ARSPD_FBW_MIN_DEFAULT).abs() < 1e-6);
