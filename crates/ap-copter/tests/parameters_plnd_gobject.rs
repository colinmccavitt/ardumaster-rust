//! Stock `PLND_` leftover, upstream `ArduCopter/Parameters.cpp`.
//!
//! The next Multi `GOBJECT` after `FLOW`. Upstream is `GOBJECT`.
//! Nested `AC_PrecLand` `var_info` is not this leftover. Later groups, G2,
...
use ap_copter::parameters::{
    ...
    find_mot_gobject_var, find_mount_gobject_var, find_plnd_gobject_var, find_psc_gobject_var,
    ...
    for_each_plnd_gobject_param_info, plnd_gobject_var_info_entry, AHRS_GOBJECT_VAR_INFO,
    ...
    GPS_GOBJECT_VAR_INFO, INS_GOBJECT_VAR_INFO, K_PARAM_ADSB, K_PARAM_OPTFLOW, K_PARAM_PRECLAND,
...
#[test]
fn table_starts_with_plnd() {
    let first = plnd_gobject_var_info_entry().expect("PLND_");
    assert_eq!(first.name, "PLND_");
    assert_eq!(first.key, K_PARAM_PRECLAND);
    assert_eq!(first.key, 74);
...
fn slice_is_one_stock_gobject() {
    assert_eq!(PLND_GOBJECT_VAR_INFO.len(), 1);
    let names: Vec<_> = PLND_GOBJECT_VAR_INFO.iter().map(|e| e.name).collect();
    assert_eq!(names, ["PLND_"]);
...
fn keys_match_the_parameters_enum() {
    let entry = find_plnd_gobject_var("PLND_").expect("PLND_");
    assert_eq!(entry.key, K_PARAM_PRECLAND);
    assert_eq!(entry.key, 74);
}

#[test]
fn plnd_is_not_the_flow_or_adsb_key() {
    assert_eq!(K_PARAM_OPTFLOW, 58);
    assert_eq!(K_PARAM_ADSB, 72);
    assert_ne!(K_PARAM_PRECLAND, K_PARAM_OPTFLOW);
...
#[test]
fn sits_after_flow_in_table_order() {
    ...
    // `FLOW` is 58; `PLND_` is 74. `ADSB_` is 72 and sits later
    // than `PLND_` on the compiled Multi table.
...
fn earlier_leftovers_do_not_include_this_slice() {
    ...
    assert!(find_rngfnd_gobject_var("PLND_").is_none());
    assert!(find_rssi_gobject_var("PLND_").is_none());
    assert!(find_mis_gobject_var("PLND_").is_none());
    assert!(find_ek3_gobject_var("PLND_").is_none());
    assert!(find_ek2_gobject_var("PLND_").is_none());
    assert!(find_rcmap_gobject_var("PLND_").is_none());
    assert!(find_mot_gobject_var("PLND_").is_none());
    assert!(find_rally_gobject_var("PLND_").is_none());
    assert!(find_avoid_gobject_var("PLND_").is_none());
    assert!(find_sched_gobject_var("PLND_").is_none());
    assert!(find_gps_gobject_var("PLND_").is_none());
    assert!(find_baro_gobject_var("PLND_").is_none());
    assert!(find_sim_gobject_var("PLND_").is_none());
    assert!(find_spray_gobject_var("PLND_").is_none());
    assert!(find_can_gobject_var("PLND_").is_none());
    assert!(find_brd_gobject_var("PLND_").is_none());
    assert!(find_batt_gobject_var("PLND_").is_none());
    assert!(find_mount_gobject_var("PLND_").is_none());
    assert!(find_ahrs_gobject_var("PLND_").is_none());
    assert!(find_psc_gobject_var("PLND_").is_none());
    assert!(find_atc_gobject_var("PLND_").is_none());
    assert!(find_wp_loit_circle_gobject_var("PLND_").is_none());
    assert!(find_ins_gobject_var("PLND_").is_none());
    assert!(find_compass_gobject_var("PLND_").is_none());
    assert!(find_relay_gobject_var("PLND_").is_none());
    assert!(find_disarm_gobject_var("PLND_").is_none());
    assert!(find_log_gobject_var("PLND_").is_none());
    assert!(find_tune_var("PLND_").is_none());
}

#[test]
fn later_rows_are_not_in_this_slice() {
    assert!(find_plnd_gobject_var("FLOW").is_none());
    assert!(find_plnd_gobject_var("H_").is_none());
    assert!(find_plnd_gobject_var("IM_").is_none());
    assert!(find_plnd_gobject_var("ADSB_").is_none());
    assert!(find_plnd_gobject_var("AVD_").is_none());
    assert!(find_plnd_gobject_var("TUNE_MIN").is_none());
}

#[test]
fn adsb_key_is_not_this_leftover() {
    assert_eq!(K_PARAM_ADSB, 72);
    let entry = find_plnd_gobject_var("PLND_").expect("PLND_");
    assert_ne!(entry.key, K_PARAM_ADSB);
    assert!(find_plnd_gobject_var("ADSB_").is_none());
...
fn ap_param_does_not_find_the_empty_group() {
    ...
    let mut n = 0_usize;
    for_each_plnd_gobject_param_info(&mut |info| {
        table[n] = info;
        n += 1;
    });
    assert_eq!(n, 1);
    ...
    // Nested `AC_PrecLand` `var_info` is not this leftover, so the
