//! Stock `SPRAY_` leftover, upstream `ArduCopter/Parameters.cpp`.
//!
//! The next Multi `GOBJECT` after `CAN_`. `SPRAY_` is
//! `HAL_SPRAYER_ENABLED`. Nested `AC_Sprayer` `var_info`
...
use ap_copter::parameters::{
    ...
    for_each_spray_gobject_param_info, spray_gobject_var_info_entry, AHRS_GOBJECT_VAR_INFO,
    ...
    K_PARAM_CAN_MGR, K_PARAM_SITL, K_PARAM_SPRAYER, LOG_GOBJECT_VAR_INFO, MOUNT_GOBJECT_VAR_INFO,
    PSC_GOBJECT_VAR_INFO, RELAY_GOBJECT_VAR_INFO, SPRAY_GOBJECT_VAR_INFO, TUNE_VAR_INFO,
...
#[test]
fn table_starts_with_spray() {
    let first = spray_gobject_var_info_entry().expect("SPRAY_");
    assert_eq!(first.name, "SPRAY_");
    assert_eq!(first.key, K_PARAM_SPRAYER);
    assert_eq!(first.key, 33);
...
fn slice_is_one_stock_gobject() {
    assert_eq!(SPRAY_GOBJECT_VAR_INFO.len(), 1);
    let names: Vec<_> = SPRAY_GOBJECT_VAR_INFO.iter().map(|e| e.name).collect();
    assert_eq!(names, ["SPRAY_"]);
...
fn keys_match_the_parameters_enum() {
    let entry = find_spray_gobject_var("SPRAY_").expect("SPRAY_");
    assert_eq!(entry.key, K_PARAM_SPRAYER);
    assert_eq!(entry.key, 33);
}

#[test]
fn spray_is_not_the_can_or_sim_key() {
    assert_eq!(K_PARAM_CAN_MGR, 8);
    assert_eq!(K_PARAM_SITL, 10);
    assert_ne!(K_PARAM_SPRAYER, K_PARAM_CAN_MGR);
    assert_ne!(K_PARAM_SPRAYER, K_PARAM_SITL);
    let entry = find_spray_gobject_var("SPRAY_").expect("SPRAY_");
...
#[test]
fn sits_after_can_in_table_order() {
    let can = find_can_gobject_var("CAN_").expect("CAN_");
    let spray = find_spray_gobject_var("SPRAY_").expect("SPRAY_");
    assert_eq!(can.key, K_PARAM_CAN_MGR);
    assert_eq!(can.ptype, VarType::Group);
    assert_eq!(spray.ptype, VarType::Group);
    // `CAN_` is 8; `SPRAY_` is a later enum slot that still sits
    // after `CAN_` on the compiled Multi table.
    assert!(spray.key > can.key);
}

#[test]
fn earlier_leftovers_do_not_include_this_slice() {
    assert!(find_can_gobject_var("SPRAY_").is_none());
    assert!(find_brd_gobject_var("SPRAY_").is_none());
    assert!(find_batt_gobject_var("SPRAY_").is_none());
    assert!(find_mount_gobject_var("SPRAY_").is_none());
    assert!(find_ahrs_gobject_var("SPRAY_").is_none());
    assert!(find_psc_gobject_var("SPRAY_").is_none());
    assert!(find_atc_gobject_var("SPRAY_").is_none());
    assert!(find_wp_loit_circle_gobject_var("SPRAY_").is_none());
    assert!(find_ins_gobject_var("SPRAY_").is_none());
    assert!(find_compass_gobject_var("SPRAY_").is_none());
    assert!(find_relay_gobject_var("SPRAY_").is_none());
    assert!(find_disarm_gobject_var("SPRAY_").is_none());
    assert!(find_log_gobject_var("SPRAY_").is_none());
    assert!(find_tune_var("SPRAY_").is_none());
}

#[test]
fn later_rows_are_not_in_this_slice() {
    assert!(find_spray_gobject_var("CAN_").is_none());
    assert!(find_spray_gobject_var("IM_").is_none());
    assert!(find_spray_gobject_var("SIM_").is_none());
    assert!(find_spray_gobject_var("TUNE_MIN").is_none());
}

#[test]
fn sitl_key_is_not_this_leftover() {
    assert_eq!(K_PARAM_SITL, 10);
    let entry = find_spray_gobject_var("SPRAY_").expect("SPRAY_");
    assert_ne!(entry.key, K_PARAM_SITL);
    assert!(find_spray_gobject_var("SIM_").is_none());
...
fn ap_param_does_not_find_the_empty_group() {
    ...
    let mut n = 0_usize;
    for_each_spray_gobject_param_info(&mut |info| {
        table[n] = info;
        n += 1;
    });
    assert_eq!(n, 1);
    ...
    // Nested `AC_Sprayer` `var_info` is not this leftover, so the
