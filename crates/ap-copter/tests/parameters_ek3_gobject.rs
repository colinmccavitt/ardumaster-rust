//! Stock `EK3_` leftover, upstream `ArduCopter/Parameters.cpp`.
//!
//! The next Multi `GOBJECT` after `EK2_`. Upstream is `GOBJECTN`.
//! Nested `NavEKF3` `var_info` is not this leftover. Later groups, G2,
...
use ap_copter::parameters::{
    ek3_gobject_var_info_entry, find_ahrs_gobject_var, find_atc_gobject_var,
    ...
    find_ek3_gobject_var, find_gps_gobject_var, find_ins_gobject_var, find_log_gobject_var,
    ...
    for_each_ek3_gobject_param_info, AHRS_GOBJECT_VAR_INFO, ATC_GOBJECT_VAR_INFO,
    ...
    K_PARAM_MISSION, K_PARAM_NAVEKF2, K_PARAM_NAVEKF3, LOG_GOBJECT_VAR_INFO, MOT_GOBJECT_VAR_INFO,
...
#[test]
fn table_starts_with_ek3() {
    let first = ek3_gobject_var_info_entry().expect("EK3_");
    assert_eq!(first.name, "EK3_");
    assert_eq!(first.key, K_PARAM_NAVEKF3);
...
fn slice_is_one_stock_gobject() {
    assert_eq!(EK3_GOBJECT_VAR_INFO.len(), 1);
    let names: Vec<_> = EK3_GOBJECT_VAR_INFO.iter().map(|e| e.name).collect();
    assert_eq!(names, ["EK3_"]);
...
fn keys_match_the_parameters_enum() {
    let entry = find_ek3_gobject_var("EK3_").expect("EK3_");
    assert_eq!(entry.key, K_PARAM_NAVEKF3);
    assert_eq!(entry.key, 7);
}

#[test]
fn ek3_is_not_the_ek2_or_mission_key() {
    assert_eq!(K_PARAM_NAVEKF2, 5);
    assert_eq!(K_PARAM_MISSION, 42);
    assert_ne!(K_PARAM_NAVEKF3, K_PARAM_NAVEKF2);
    assert_ne!(K_PARAM_NAVEKF3, K_PARAM_MISSION);
    let entry = find_ek3_gobject_var("EK3_").expect("EK3_");
...
#[test]
fn sits_after_ek2_in_table_order() {
    ...
    // `EK2_` is 5; `EK3_` is 7. Both sit after `RCMAP_` on the compiled
    // Multi table even though the keys are early enum slots.
    assert!(ek2.key < ek3.key);
}

#[test]
fn earlier_leftovers_do_not_include_this_slice() {
    assert!(find_ek2_gobject_var("EK3_").is_none());
    assert!(find_rcmap_gobject_var("EK3_").is_none());
    assert!(find_mot_gobject_var("EK3_").is_none());
    assert!(find_rally_gobject_var("EK3_").is_none());
    assert!(find_avoid_gobject_var("EK3_").is_none());
    assert!(find_sched_gobject_var("EK3_").is_none());
    assert!(find_gps_gobject_var("EK3_").is_none());
    assert!(find_baro_gobject_var("EK3_").is_none());
    assert!(find_sim_gobject_var("EK3_").is_none());
    assert!(find_spray_gobject_var("EK3_").is_none());
    assert!(find_can_gobject_var("EK3_").is_none());
    assert!(find_brd_gobject_var("EK3_").is_none());
    assert!(find_batt_gobject_var("EK3_").is_none());
    assert!(find_mount_gobject_var("EK3_").is_none());
    assert!(find_ahrs_gobject_var("EK3_").is_none());
    assert!(find_psc_gobject_var("EK3_").is_none());
    assert!(find_atc_gobject_var("EK3_").is_none());
    assert!(find_wp_loit_circle_gobject_var("EK3_").is_none());
    assert!(find_ins_gobject_var("EK3_").is_none());
    assert!(find_compass_gobject_var("EK3_").is_none());
    assert!(find_relay_gobject_var("EK3_").is_none());
    assert!(find_disarm_gobject_var("EK3_").is_none());
    assert!(find_log_gobject_var("EK3_").is_none());
    assert!(find_tune_var("EK3_").is_none());
}

#[test]
fn later_rows_are_not_in_this_slice() {
    assert!(find_ek3_gobject_var("EK2_").is_none());
    assert!(find_ek3_gobject_var("H_").is_none());
    assert!(find_ek3_gobject_var("IM_").is_none());
    assert!(find_ek3_gobject_var("MIS_").is_none());
    assert!(find_ek3_gobject_var("TUNE_MIN").is_none());
}

#[test]
fn mission_key_is_not_this_leftover() {
    assert_eq!(K_PARAM_MISSION, 42);
    let entry = find_ek3_gobject_var("EK3_").expect("EK3_");
    assert_ne!(entry.key, K_PARAM_MISSION);
    assert!(find_ek3_gobject_var("MIS_").is_none());
...
fn ap_param_does_not_find_the_empty_group() {
    ...
    let mut n = 0_usize;
    for_each_ek3_gobject_param_info(&mut |info| {
        table[n] = info;
        n += 1;
    });
    assert_eq!(n, 1);
    ...
    // Nested `NavEKF3` `var_info` is not this leftover, so the
