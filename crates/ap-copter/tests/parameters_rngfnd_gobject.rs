//! Stock `RNGFND` leftover, upstream `ArduCopter/Parameters.cpp`.
//!
//! The next Multi `GOBJECT` after `RSSI_`. Upstream is `GOBJECT`.
//! Nested `RangeFinder` `var_info` is not this leftover. Later groups, G2,
...
use ap_copter::parameters::{
    ...
    for_each_rngfnd_gobject_param_info, rngfnd_gobject_var_info_entry, AHRS_GOBJECT_VAR_INFO,
    ...
    INS_GOBJECT_VAR_INFO, K_PARAM_RANGEFINDER, K_PARAM_RSSI, K_PARAM_TERRAIN, LOG_GOBJECT_VAR_INFO,
...
#[test]
fn table_starts_with_rngfnd() {
    let first = rngfnd_gobject_var_info_entry().expect("RNGFND");
    assert_eq!(first.name, "RNGFND");
    assert_eq!(first.key, K_PARAM_RANGEFINDER);
    assert_eq!(first.key, 53);
...
fn slice_is_one_stock_gobject() {
    assert_eq!(RNGFND_GOBJECT_VAR_INFO.len(), 1);
    let names: Vec<_> = RNGFND_GOBJECT_VAR_INFO.iter().map(|e| e.name).collect();
    assert_eq!(names, ["RNGFND"]);
...
fn keys_match_the_parameters_enum() {
    let entry = find_rngfnd_gobject_var("RNGFND").expect("RNGFND");
    assert_eq!(entry.key, K_PARAM_RANGEFINDER);
    assert_eq!(entry.key, 53);
}

#[test]
fn rngfnd_is_not_the_rssi_or_terrain_key() {
    assert_eq!(K_PARAM_RSSI, 97);
    assert_eq!(K_PARAM_TERRAIN, 55);
    assert_ne!(K_PARAM_RANGEFINDER, K_PARAM_RSSI);
    assert_ne!(K_PARAM_RANGEFINDER, K_PARAM_TERRAIN);
    let entry = find_rngfnd_gobject_var("RNGFND").expect("RNGFND");
...
#[test]
fn sits_after_rssi_in_table_order() {
    ...
    // `RSSI_` is 97; `RNGFND` is 53. `RNGFND` sits later than `RSSI_`
    // on the compiled Multi table even though its key is smaller.
    // `TERRAIN_` is 55 and sits later than `RNGFND`.
...
fn earlier_leftovers_do_not_include_this_slice() {
    ...
    assert!(find_mis_gobject_var("RNGFND").is_none());
    assert!(find_ek3_gobject_var("RNGFND").is_none());
    assert!(find_ek2_gobject_var("RNGFND").is_none());
    assert!(find_rcmap_gobject_var("RNGFND").is_none());
    assert!(find_mot_gobject_var("RNGFND").is_none());
    assert!(find_rally_gobject_var("RNGFND").is_none());
    assert!(find_avoid_gobject_var("RNGFND").is_none());
    assert!(find_sched_gobject_var("RNGFND").is_none());
    assert!(find_gps_gobject_var("RNGFND").is_none());
    assert!(find_baro_gobject_var("RNGFND").is_none());
    assert!(find_sim_gobject_var("RNGFND").is_none());
    assert!(find_spray_gobject_var("RNGFND").is_none());
    assert!(find_can_gobject_var("RNGFND").is_none());
    assert!(find_brd_gobject_var("RNGFND").is_none());
    assert!(find_batt_gobject_var("RNGFND").is_none());
    assert!(find_mount_gobject_var("RNGFND").is_none());
    assert!(find_ahrs_gobject_var("RNGFND").is_none());
    assert!(find_psc_gobject_var("RNGFND").is_none());
    assert!(find_atc_gobject_var("RNGFND").is_none());
    assert!(find_wp_loit_circle_gobject_var("RNGFND").is_none());
    assert!(find_ins_gobject_var("RNGFND").is_none());
    assert!(find_compass_gobject_var("RNGFND").is_none());
    assert!(find_relay_gobject_var("RNGFND").is_none());
    assert!(find_disarm_gobject_var("RNGFND").is_none());
    assert!(find_log_gobject_var("RNGFND").is_none());
    assert!(find_tune_var("RNGFND").is_none());
}

#[test]
fn later_rows_are_not_in_this_slice() {
    assert!(find_rngfnd_gobject_var("RSSI_").is_none());
    assert!(find_rngfnd_gobject_var("H_").is_none());
    assert!(find_rngfnd_gobject_var("IM_").is_none());
    assert!(find_rngfnd_gobject_var("TERRAIN_").is_none());
    assert!(find_rngfnd_gobject_var("FLOW").is_none());
    assert!(find_rngfnd_gobject_var("TUNE_MIN").is_none());
}

#[test]
fn terrain_key_is_not_this_leftover() {
    assert_eq!(K_PARAM_TERRAIN, 55);
    let entry = find_rngfnd_gobject_var("RNGFND").expect("RNGFND");
    assert_ne!(entry.key, K_PARAM_TERRAIN);
    assert!(find_rngfnd_gobject_var("TERRAIN_").is_none());
...
fn ap_param_does_not_find_the_empty_group() {
    ...
    let mut n = 0_usize;
    for_each_rngfnd_gobject_param_info(&mut |info| {
        table[n] = info;
        n += 1;
    });
    assert_eq!(n, 1);
    ...
    // Nested `RangeFinder` `var_info` is not this leftover, so the
