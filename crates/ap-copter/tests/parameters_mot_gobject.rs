//! Stock `MOT_` leftover, upstream `ArduCopter/Parameters.cpp`.
//!
//! The next Multi `GOBJECT` after `RALLY_`. Upstream is
//! `GOBJECTVARPTR`. Nested `AP_MotorsMulticopter` `var_info` is not this
//! leftover. Later groups, G2, and `load_parameters` stay later. Heli
...
use ap_copter::parameters::{
    ...
    find_tune_var, find_wp_loit_circle_gobject_var, for_each_mot_gobject_param_info,
    mot_gobject_var_info_entry, AHRS_GOBJECT_VAR_INFO, ATC_GOBJECT_VAR_INFO,
    ...
    GPS_GOBJECT_VAR_INFO, INS_GOBJECT_VAR_INFO, K_PARAM_MOTORS, K_PARAM_RALLY, K_PARAM_RCMAP,
    LOG_GOBJECT_VAR_INFO, MOT_GOBJECT_VAR_INFO, MOUNT_GOBJECT_VAR_INFO, PSC_GOBJECT_VAR_INFO,
    RALLY_GOBJECT_VAR_INFO, RELAY_GOBJECT_VAR_INFO, SCHED_GOBJECT_VAR_INFO, SIM_GOBJECT_VAR_INFO,
...
#[test]
fn table_starts_with_mot() {
    let first = mot_gobject_var_info_entry().expect("MOT_");
    assert_eq!(first.name, "MOT_");
    assert_eq!(first.key, K_PARAM_MOTORS);
    assert_eq!(first.key, 90);
...
fn slice_is_one_stock_gobject() {
    assert_eq!(MOT_GOBJECT_VAR_INFO.len(), 1);
    let names: Vec<_> = MOT_GOBJECT_VAR_INFO.iter().map(|e| e.name).collect();
    assert_eq!(names, ["MOT_"]);
...
fn keys_match_the_parameters_enum() {
    let entry = find_mot_gobject_var("MOT_").expect("MOT_");
    assert_eq!(entry.key, K_PARAM_MOTORS);
    assert_eq!(entry.key, 90);
}

#[test]
fn mot_is_not_the_rally_or_rcmap_key() {
    assert_eq!(K_PARAM_RALLY, 45);
    assert_eq!(K_PARAM_RCMAP, 199);
    assert_ne!(K_PARAM_MOTORS, K_PARAM_RALLY);
    assert_ne!(K_PARAM_MOTORS, K_PARAM_RCMAP);
    let entry = find_mot_gobject_var("MOT_").expect("MOT_");
...
#[test]
fn sits_after_rally_in_table_order() {
    ...
    // `RALLY_` is 45; `MOT_` is a later enum slot and sits after `RALLY_`
    // on the compiled Multi table.
    assert!(mot.key > rally.key);
}

#[test]
fn earlier_leftovers_do_not_include_this_slice() {
    assert!(find_rally_gobject_var("MOT_").is_none());
    assert!(find_avoid_gobject_var("MOT_").is_none());
    assert!(find_sched_gobject_var("MOT_").is_none());
    assert!(find_gps_gobject_var("MOT_").is_none());
    assert!(find_baro_gobject_var("MOT_").is_none());
    assert!(find_sim_gobject_var("MOT_").is_none());
    assert!(find_spray_gobject_var("MOT_").is_none());
    assert!(find_can_gobject_var("MOT_").is_none());
    assert!(find_brd_gobject_var("MOT_").is_none());
    assert!(find_batt_gobject_var("MOT_").is_none());
    assert!(find_mount_gobject_var("MOT_").is_none());
    assert!(find_ahrs_gobject_var("MOT_").is_none());
    assert!(find_psc_gobject_var("MOT_").is_none());
    assert!(find_atc_gobject_var("MOT_").is_none());
    assert!(find_wp_loit_circle_gobject_var("MOT_").is_none());
    assert!(find_ins_gobject_var("MOT_").is_none());
    assert!(find_compass_gobject_var("MOT_").is_none());
    assert!(find_relay_gobject_var("MOT_").is_none());
    assert!(find_disarm_gobject_var("MOT_").is_none());
    assert!(find_log_gobject_var("MOT_").is_none());
    assert!(find_tune_var("MOT_").is_none());
}

#[test]
fn later_rows_are_not_in_this_slice() {
    assert!(find_mot_gobject_var("RALLY_").is_none());
    assert!(find_mot_gobject_var("H_").is_none());
    assert!(find_mot_gobject_var("IM_").is_none());
    assert!(find_mot_gobject_var("RCMAP_").is_none());
    assert!(find_mot_gobject_var("TUNE_MIN").is_none());
}

#[test]
fn rcmap_key_is_not_this_leftover() {
    assert_eq!(K_PARAM_RCMAP, 199);
    let entry = find_mot_gobject_var("MOT_").expect("MOT_");
    assert_ne!(entry.key, K_PARAM_RCMAP);
    assert!(find_mot_gobject_var("RCMAP_").is_none());
...
fn ap_param_does_not_find_the_empty_group() {
    ...
    let mut n = 0_usize;
    for_each_mot_gobject_param_info(&mut |info| {
        table[n] = info;
        n += 1;
    });
    assert_eq!(n, 1);
    ...
    // Nested `AP_MotorsMulticopter` `var_info` is not this leftover, so the
