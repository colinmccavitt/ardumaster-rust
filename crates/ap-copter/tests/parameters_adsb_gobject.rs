//! Stock `ADSB_` leftover, upstream `ArduCopter/Parameters.cpp`.
//!
//! The next Multi `GOBJECT` after `PLND_`. Upstream is `GOBJECT`.
//! Nested `AP_ADSB` `var_info` is not this leftover. Later groups, G2,
...
use ap_copter::parameters::{
    adsb_gobject_var_info_entry, find_adsb_gobject_var, find_ahrs_gobject_var,
    find_atc_gobject_var, find_avoid_gobject_var, find_baro_gobject_var, find_batt_gobject_var,
    ...
    find_tune_var, find_wp_loit_circle_gobject_var, for_each_adsb_gobject_param_info,
    ADSB_GOBJECT_VAR_INFO, AHRS_GOBJECT_VAR_INFO, ATC_GOBJECT_VAR_INFO, AVOID_GOBJECT_VAR_INFO,
    ...
    K_PARAM_ADSB, K_PARAM_AVOIDANCE_ADSB, K_PARAM_PRECLAND, LOG_GOBJECT_VAR_INFO,
    MIS_GOBJECT_VAR_INFO, MOT_GOBJECT_VAR_INFO, MOUNT_GOBJECT_VAR_INFO, PLND_GOBJECT_VAR_INFO,
    ...
    SPRAY_GOBJECT_VAR_INFO, TERRAIN_GOBJECT_VAR_INFO, TUNE_VAR_INFO,
...
#[test]
fn table_starts_with_adsb() {
    let first = adsb_gobject_var_info_entry().expect("ADSB_");
    assert_eq!(first.name, "ADSB_");
    assert_eq!(first.key, K_PARAM_ADSB);
    assert_eq!(first.key, 72);
...
fn slice_is_one_stock_gobject() {
    assert_eq!(ADSB_GOBJECT_VAR_INFO.len(), 1);
    let names: Vec<_> = ADSB_GOBJECT_VAR_INFO.iter().map(|e| e.name).collect();
    assert_eq!(names, ["ADSB_"]);
...
fn keys_match_the_parameters_enum() {
    let entry = find_adsb_gobject_var("ADSB_").expect("ADSB_");
    assert_eq!(entry.key, K_PARAM_ADSB);
    assert_eq!(entry.key, 72);
}

#[test]
fn adsb_is_not_the_plnd_or_avd_key() {
    assert_eq!(K_PARAM_PRECLAND, 74);
    assert_eq!(K_PARAM_AVOIDANCE_ADSB, 96);
    assert_ne!(K_PARAM_ADSB, K_PARAM_PRECLAND);
...
#[test]
fn sits_after_plnd_in_table_order() {
    let plnd = find_plnd_gobject_var("PLND_").expect("PLND_");
    let adsb = find_adsb_gobject_var("ADSB_").expect("ADSB_");
    assert_eq!(plnd.key, K_PARAM_PRECLAND);
    assert_eq!(adsb.key, K_PARAM_ADSB);
    assert_eq!(plnd.ptype, VarType::Group);
    assert_eq!(adsb.ptype, VarType::Group);
    // `PLND_` is 74; `ADSB_` is 72 and sits later than `PLND_`
    // on the compiled Multi table. `AVD_` is 96 and stays later.
    assert_ne!(K_PARAM_PRECLAND, K_PARAM_ADSB);
    assert_ne!(K_PARAM_ADSB, K_PARAM_AVOIDANCE_ADSB);
}

#[test]
fn earlier_leftovers_do_not_include_this_slice() {
    assert!(find_plnd_gobject_var("ADSB_").is_none());
    assert!(find_flow_gobject_var("ADSB_").is_none());
    assert!(find_terrain_gobject_var("ADSB_").is_none());
    assert!(find_rngfnd_gobject_var("ADSB_").is_none());
    assert!(find_rssi_gobject_var("ADSB_").is_none());
    assert!(find_mis_gobject_var("ADSB_").is_none());
    assert!(find_ek3_gobject_var("ADSB_").is_none());
    assert!(find_ek2_gobject_var("ADSB_").is_none());
    assert!(find_rcmap_gobject_var("ADSB_").is_none());
    assert!(find_mot_gobject_var("ADSB_").is_none());
    assert!(find_rally_gobject_var("ADSB_").is_none());
    assert!(find_avoid_gobject_var("ADSB_").is_none());
    assert!(find_sched_gobject_var("ADSB_").is_none());
    assert!(find_gps_gobject_var("ADSB_").is_none());
    assert!(find_baro_gobject_var("ADSB_").is_none());
    assert!(find_sim_gobject_var("ADSB_").is_none());
    assert!(find_spray_gobject_var("ADSB_").is_none());
    assert!(find_can_gobject_var("ADSB_").is_none());
    assert!(find_brd_gobject_var("ADSB_").is_none());
    assert!(find_batt_gobject_var("ADSB_").is_none());
    assert!(find_mount_gobject_var("ADSB_").is_none());
    assert!(find_ahrs_gobject_var("ADSB_").is_none());
    assert!(find_psc_gobject_var("ADSB_").is_none());
    assert!(find_atc_gobject_var("ADSB_").is_none());
    assert!(find_wp_loit_circle_gobject_var("ADSB_").is_none());
    assert!(find_ins_gobject_var("ADSB_").is_none());
    assert!(find_compass_gobject_var("ADSB_").is_none());
    assert!(find_relay_gobject_var("ADSB_").is_none());
    assert!(find_disarm_gobject_var("ADSB_").is_none());
    assert!(find_log_gobject_var("ADSB_").is_none());
    assert!(find_tune_var("ADSB_").is_none());
}

#[test]
fn later_rows_are_not_in_this_slice() {
    assert!(find_adsb_gobject_var("PLND_").is_none());
    assert!(find_adsb_gobject_var("H_").is_none());
    assert!(find_adsb_gobject_var("IM_").is_none());
    assert!(find_adsb_gobject_var("AVD_").is_none());
    assert!(find_adsb_gobject_var("NTF_").is_none());
    assert!(find_adsb_gobject_var("TUNE_MIN").is_none());
}

#[test]
fn avd_key_is_not_this_leftover() {
    assert_eq!(K_PARAM_AVOIDANCE_ADSB, 96);
    let entry = find_adsb_gobject_var("ADSB_").expect("ADSB_");
    assert_ne!(entry.key, K_PARAM_AVOIDANCE_ADSB);
    assert!(find_adsb_gobject_var("AVD_").is_none());
...
fn ap_param_does_not_find_the_empty_group() {
    ...
    let mut n = 0_usize;
    for_each_adsb_gobject_param_info(&mut |info| {
        table[n] = info;
        n += 1;
    });
    assert_eq!(n, 1);
    ...
    // Nested `AP_ADSB` `var_info` is not this leftover, so the
    // group contributes no children.
    assert!(find_by_name(&table, filter, "ADSB_").is_none());
