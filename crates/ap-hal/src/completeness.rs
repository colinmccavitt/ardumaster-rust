...
pub const HAL_COMPLETENESS: &[HalPortItem] = &[
    ...
    HalPortItem {
        name: "CANIface",
        status: PortStatus::ThisSlice,
        note: "AP_HAL/CANIface.h send/receive, bitrate, filter",
    },
    HalPortItem {
        name: "DSP",
        status: PortStatus::ThisSlice,
    ...
    HalPortItem {
        name: "Flash",
        status: PortStatus::ThisSlice,
        note: "AP_HAL/Flash.h erase/write/read, sector size",
    },
    HalPortItem {
        name: "OpticalFlow",
        status: PortStatus::ThisSlice,
        note: "AP_HAL/OpticalFlow.h quality/flow-rate sample; init/read/push_gyro",
    },
    HalPortItem {
...
mod tests {
    ...
    fn table_lists_gpio_semaphore_util_device_done() {
        ...
        assert!(completeness_has("CANIface", PortStatus::ThisSlice));
        assert!(completeness_has(
        ...
        assert!(completeness_has("OpticalFlow", PortStatus::ThisSlice));
        assert_eq!(on_main_items().count(), 4);
        assert_eq!(this_slice_items().count(), 8);
